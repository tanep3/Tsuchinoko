...
3.  **ModuleTable (モジュールテーブル)**: `import` されたモジュールを Bridge 内のテーブルで一元管理し、`bridge.get()` で参照する。
4.  **Maximum A (現実的拡張)**: 属性アクセス、インデクサ、バッチイテレータのみをサポートし、複雑な動的機能（メタクラス操作等）はスコープ外とする。
...
### 2.3 Python Worker 起動アーキテクチャ (Robustness)

Windows/Linux 環境差分を吸収し、トラブルシューティングを容易にするため、以下の起動シーケンスを採用する。

1.  **実行パス解決**:
    *   環境変数 `PYO3_PYTHON` があれば最優先で使用する（venv 対応）。
    *   なければ `python` コマンドを試行。
    *   最後に `python3` コマンドを試行。
2.  **診断ログ出力 (Diagnosability)**:
    *   Worker は起動直後、`stderr` に `sys.executable`, `sys.version`, `sys.path` を出力する。
    *   Rust Runtime は Worker の `stderr` を親プロセスに継承 (`Inherit`) させ、ユーザーがログを確認できるようにする。
3.  **クラッシュ検知**:
    *   `stdout` の EOF を検知した場合、単なる「EOFエラー」ではなく、「WorkerCrash: stderr を確認してください」という明確なメッセージを返す。
...
### 3.2 ModuleTable Architecture

トップレベルの import スコープ問題を解決するため、Bridge 内に `ModuleTable` を導入する。

#### A. Bridge API

```rust
impl PythonBridge {
    // import cv2 as cv -> bridge.import("cv2", "cv")
    fn import(&self, py_name: &str, alias: &str);
    
    // cv.VideoCapture -> bridge.get("cv").call_method(...)
    fn get(&self, alias: &str) -> ModuleRef;
}
```

#### B. Split Reference Strategy (Proxy)

`ModuleRef` (エイリアス参照) と `HandleRef` (オブジェクトID参照) を型レベルで分離し、操作ミスを防ぐ。
...
両者とも共通の操作メソッド (`call_method` 等) を持つが、内部処理（RPCメッセージの宛先指定など）は異なる。
これにより、将来的に「モジュールは PyO3、オブジェクトは RPC」といった混合構成にも対応可能とする。

### 3.3 遅延バインディング (Late Binding)
