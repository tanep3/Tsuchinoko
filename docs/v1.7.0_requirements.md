...
### 2.2 Module Management (ModuleTable) [New]

Python の「グローバル名前空間」を Rust 側で `ModuleTable` として再構築する。
これにより、トップレベル import が関数スコープのローカル変数に落ちてしまう問題を解決する。

*   **要件**: 全ての import 文は `ModuleTable` への登録処理に変換されること。
*   **制約**: 外部モジュールへのアクセスは必ず `bridge.get("alias")` を介して行われること。
*   **禁止**: `bridge.call_function("__import__", ...)` の戻り値を直接ローカル変数で保持して使い回すことは禁止とする（スコープ問題の解決）。
