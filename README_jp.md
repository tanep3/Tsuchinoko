...
[![Coverage](https://img.shields.io/badge/Python構文カバレッジ-59%25-blue.svg)](#機能ドキュメント)
...
> **カバレッジ**: Python構文機能の59%(100機能)をサポート。変数、演算子、制御フロー、関数、クラス、データ構造、コレクション(list/dict/set)、文字列メソッド、Result型によるロバストなエラー処理など、アルゴリズムプログラミングに必要な基本構造をカバーしています。
...
- **Setリテラル** (V1.5.0) - `{1, 2, 3}` → `HashSet`
- **コレクションメソッド** (V1.5.0) - list/dict/set操作 (pop, insert, keys, values, union等)
- **文字列メソッド** (V1.5.0) - replace, startswith, endswith, find, count, isdigit等
- **Optionalパターン** (V1.5.0) - `x or default`, Noneチェック+三項演算子
- **ステップスライス** (V1.5.0) - `arr[::2]`, `arr[::-1]`
...
- **Rust プレビュー** (`Ctrl+Alt+P`) - リアルタイムで変換結果を表示
...
- [ユーザーマニュアル (はじめに)](docs/user-manual_jp.md) | [User Manual](docs/user-manual.md)
