use tsuchinoko::bridge::{
    ...
    protocol::{JsonPrimitive, TnkValue},
    PythonBridge,
};
...
fn test_bridge_call_function_str() {
    ...
    let arg = TnkValue::Value {
        value: Some(JsonPrimitive::String("Hello".into())),
    };
    let args = [&arg];
    let result = bridge
        .call_function("builtins.str", &args, None)
        .expect("Call failed");
...
fn test_bridge_math_sqrt() {
    ...
    let args = [&arg];
    let result = bridge
        .call_function("math.sqrt", &args, None)
        .expect("Call failed");
...
fn test_bridge_list_slice() {
    ...
    let list_handle = bridge
        .call_function("builtins.list", &arg_refs, None)
    ...
    let slice_result = bridge
        .slice(&target, Some(start), Some(stop), Some(step))
        .expect("Slice failed");
...
fn test_bridge_security_blocks_eval_function() {
    ...
    let arg = TnkValue::Value {
        value: Some(JsonPrimitive::String("1+1".into())),
    };
    let args = [&arg];
    let err = bridge
        .call_function("builtins.eval", &args, None)
        .expect_err("Expected SecurityViolation");
    match err {
