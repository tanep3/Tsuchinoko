# Tsuchinoko VSCode 拡張 v0.2.0 要件定義書

> **バージョン**: v0.2.0  
> **作成日**: 2026-01-15  
> **目標リリース**: v1.7.0 完了後
> **前バージョン**: v0.1.0 (2026-01-06)
...
| 機能 | 説明 | 優先度 |
|------|------|--------|
| トランスパイル+診断 | `Ctrl+Alt+P` で変換とプレビュー、診断も同時実行 | 高 |
| Rust プレビュー | サイドパネルに変換結果を表示 | 高 |
| 正確な診断 (V0.2.0) | `--diag-json` による column-accurate な波線表示 | 高 |
| 診断コード表示 (V0.2.0) | `TNK-UNSUPPORTED-SYNTAX` 等のコード付与 | 中 |
| ステータスバー表示 | 変換可否を表示 (✅/❌) | 低 |

**V0.2.0の主な変更**:
- ❌ **削除**: 保存時の自動チェック機能
- ✅ **追加**: `--diag-json` による正確な診断位置表示
- ✅ **改善**: プレビュー実行時のみ診断を実行（UX改善）
...
| `tsuchinoko.preview` | Tsuchinoko: Show Rust Preview | `Ctrl+Alt+P` / `Cmd+Alt+P` |
...
**V0.2.0で削除された設定**:
- `tsuchinoko.autoCheck` (削除理由: プレビュー時のみ診断に変更)
- `tsuchinoko.checkDelay` (削除理由: 不要になった)
